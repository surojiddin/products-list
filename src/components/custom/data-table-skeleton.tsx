					...
					<TableHeader>
						<TableRow className="hover:bg-transparent">
							{Array.from({ length: columnCount }).map(() => (
								<TableHead
									key={generateKey('header')}
									style={{
										width: cellWidths[0], // Using the first width for all headers for simplicity
				...
				<div className="flex w-full items-center justify-between gap-4 overflow-auto p-1 sm:gap-8">
					...
					<div className="flex items-center gap-4 sm:gap-6 lg:gap-8">
						<div className="flex items-center space-x-2">
							<Skeleton
								key={generateKey('pagination-1')}
								className="h-7 w-24"
							/>
							<Skeleton
								key={generateKey('pagination-2')}
								className="h-7 w-[4.5rem]"
							/>
						</div>
						<div className="flex items-center justify-center font-medium text-sm">
							<Skeleton
								key={generateKey('pagination-3')}
								className="h-7 w-20"
							/>
						</div>
						<div className="flex items-center space-x-2">
							<Skeleton
								key={generateKey('pagination-4')}
								className="hidden size-7 lg:block"
							/>
							<Skeleton key={generateKey('pagination-5')} className="size-7" />
							<Skeleton key={generateKey('pagination-6')} className="size-7" />
							<Skeleton
								key={generateKey('pagination-7')}
								className="hidden size-7 lg:block"
							/>
						</div>
					</div>
